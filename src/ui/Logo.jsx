import React from "react";
import { BiBookOpen } from "react-icons/bi";

export default function Logo() {
  return (
    <div className="relative flex gap-2 before:absolute before:-bottom-6 before:-left-4 before:h-px before:w-[calc(100%+32px)] before:bg-stone-100/20 before:content-['']">
      <div className="bg-accent flex h-max w-max items-center justify-center rounded-xl p-2.5">
        <BiBookOpen className="text-primary text-2xl" />
      </div>
      <div>
        <h3 className="text-primary text-lg font-bold">EduRev</h3>
        <p className="text-xs text-stone-300">Insight Platform</p>
      </div>
    </div>
  );
}
